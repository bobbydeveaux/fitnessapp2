...
resource "aws_cloudfront_distribution" "s3_distribution" {
  ...
  aliases = ["fitnessapp.tld", "www.fitnessapp.tld"] 
...
terraform {
   backend "s3" {
     encrypt = true
     bucket = "fitnessapp-tf-remote-state-storage-s3"
     region = "eu-west-2"
     dynamodb_table = "fitnessapp-tf-state-lock-dynamo"
     key = "state"
   }
}
...
resource "aws_dynamodb_table" "dynamodb-terraform-state-lock" {
  name = "fitnessapp-tf-state-lock-dynamo"
  hash_key = "LockID"
  read_capacity = 20
  write_capacity = 20
